# -*- coding: utf-8 -*-
"""
select_pairs_and_donors.py  (versión optimizada V2: sharding por tienda + scoring por tienda)

- Mantiene:
  * Misma firma de select_pairs_and_donors(H, train, items, stores, outdir)
  * Mismos nombres de funciones públicas/privadas originales (compatibilidad)
  * Mismos archivos de salida y logs/progreso clave
- Acelera:
  * Scoring ΔH_cls: unidad de trabajo = TIENDA (carga única por tienda) + vectorización por clase
  * Evita relecturas del CSV completo de H por cada i/j
  * Cache de grupo ahora lee de shard de tienda (si existe)
- Añade:
  * Sharding por tienda (una pasada de H) a outdir/shards
  * Muestreo estratificado sin groupby.apply (sin FutureWarning)
"""
...
# ------------------- Parámetros (idénticos + override opcional por ENV) -------------------
...
CHUNK_H = _env_int("SPD_CHUNK_H", 4_000_000)
CHUNK_TRAIN = _env_int("SPD_CHUNK_TRAIN", 4_000_000)

# Selección de universo base
MIN_ITEM_OBS = 200
H_SD_MIN = 0.004
P_ANY_MIN, P_ANY_MAX = 0.02, 0.98

# Caníbales i
P_PROMO_I_MIN, P_PROMO_I_MAX = 0.03, 0.25
WINDOW_START = pd.Timestamp("2016-01-01")
WINDOW_END   = pd.Timestamp("2017-06-30")
MIN_RUN_DAYS = 3
# Override por env
...
N_WORKERS = max(1, min(_env_int("SPD_N_CORES", 6), os.cpu_count() or 1))
...
SPD_MAX_TASKS_SCORE  = _env_int("SPD_MAX_TASKS_SCORE", 20000)     # tope global
...
def _build_H_store_shards(H_use: str, stores_of_interest: List[int]) -> None:
    ...
    for ch in readerH:
        ...
        for s, df_s in ch.groupby("store_nbr"):
            ...
            # pandas maneja múltiples miembros gzip correctamente
...
# ------------------- Cache de grupo (ahora prefiere shard de tienda) -------------------
def _get_H_group_cache(H_use: str, store_nbr: int, cls: int) -> Dict:
    """Devuelve dict con dates_np, items, Hmat, S, C, item_metrics. Prefiere shard de tienda si existe."""
    key = (H_use, int(store_nbr), int(cls))
    if key in _PROC_CACHE_HSC:
        _lru_touch(key)
        return _PROC_CACHE_HSC[key]

    # Decide fuente: shard por tienda (si existe) o CSV grande
    ...
    cache = {
        ...
        # "J_ON": se añade después si hace falta
    }
    _PROC_CACHE_HSC[key] = cache; _lru_touch(key); _lru_prune()
    return cache
...
# ------------------- API compatibles (algunas reimplementadas arriba) -------------------
...
# ------------------- Workers originales (se mantienen por compatibilidad) -------------------
def _score_cannibal_worker(args) -> Optional[Dict]:
    """Se mantiene por compatibilidad, no se usa en la ruta optimizada."""
...
# ------------------- NUEVOS workers por TIENDA -------------------
def _score_cannibals_store_worker(args) -> List[Dict]:
    """
    Worker por TIENDA:
      - Carga shard de H de esa tienda una sola vez
      - Carga train de esa tienda una sola vez
      - Para cada clase -> computa ΔH_cls para TODOS los i candidatos de esa clase
    args: (H_use, train_csv, store_nbr, class_to_items_map)
    """
    H_use, train_csv, s, class_to_items = args
    out_rows: List[Dict] = []
    try:
        # Prepara train de tienda
        train = _get_train_cache(train_csv, s)
        runs_map = train["runs_by_item"]

        # Para cada clase del mapa: usa el cache de grupo (que ahora lee del shard de tienda)
...
def select_pairs_and_donors(H_csv: str, train_csv: str, items_csv: str, stores_csv: str,
    ...
    _LOGGER.info("=== Inicio select_pairs_and_donors (optimizado V2) ===")
    _LOGGER.info(f"Parámetros: N_WORKERS={N_WORKERS}, CHUNK_H={CHUNK_H:,}, CHUNK_TRAIN={CHUNK_TRAIN:,}")
    _LOGGER.info(f"ENV sampling: SPD_SAMPLE_CI_PER_SC={SPD_SAMPLE_CI_PER_SC}, SPD_SAMPLE_FRAC_CI={SPD_SAMPLE_FRAC_CI}, SPD_MAX_TASKS_SCORE={SPD_MAX_TASKS_SCORE}")
    ...
    # Ordenamos por (store,class) para mejorar cachés
    ...
    # Limita procesos al nº de tiendas
    n_workers = max(1, min(N_WORKERS, len(store_tasks)))
    with mp.Pool(processes=n_workers) as pool:
        for rows in pool.imap_unordered(_score_cannibals_store_worker, store_tasks, chunksize=1):
            if rows:
                cannibals.extend(rows)
                processed_i += sum(1 for _ in rows)  # aproximación
            pct = 12.0 + (processed_i / max(1, total_i)) * 88.0
            _progress_emit("score_cannibals", min(99.0, pct),
                           f"Caníbales procesados (aprox ítems): {min(processed_i,total_i)}/{total_i}")

    if not cannibals:
        _progress_emit("score_cannibals", 100.0, "No se encontraron caníbales con episodios válidos.", force_log=True)
        raise RuntimeError("No se encontraron caníbales con episodios y ΔH_cls en la ventana 2016–2017.")

    can_df = (pd.DataFrame(cannibals)
                .sort_values("delta_H_cls", ascending=False)
                .head(N_CANNIBALS)
    ...
    if not can_df.empty:
        ...
        total_v = len(v_tasks)
        done_v = 0
        # ordenar por (store,class) para aprovechar shards/caché
        v_tasks.sort(key=lambda t: (t[2], t[4], t[3]))
    ...
    # --- 4) Donantes (igual que antes; no es cuello)
    ...
    profiles = _aggregate_item_profile(H_use)
    PROGRESS_FLAGS["mute_internal_progress"] = False
    ...
    for _, v in pairs_df.iterrows():
        ...
        pct = 100.0 * processed_pairs / max(1, total_pairs)
